...
@onready var melee_weapon2 = get_node("../Player2/MeleeWeapon2")
@onready var ranged_weapon2 = get_node("../Player2/RangedWeapon2")

@onready var melee_weapon1 = get_node("MeleeWeapon1")
@onready var melee_weapon_sprite1 = get_node("MeleeWeapon1/Sprite2D")

@onready var ranged_weapon1 = get_node("RangedWeapon1")

@onready var BlueRifle = get_node("../BlueRifle")
@onready var BlueSaber = get_node("../BlueSaber")
...
func player_idle(delta): # The Print function in here if active will flood the output, making debugging hard
	if velocity.x == 0 and is_on_floor() and current_state != State.Crouch and current_state != State.Gun_Shoot and current_state != State.Sword_Attack and current_state != State.Punch:
		current_state = State.Idle
		#print("101, Idle")
		if ranged_weapon1.visible:
			current_state = State.Idle_Gun
			#print("105, Idle_Gun")
		elif melee_weapon1.visible:
...
func player_walk(delta):
	...
	if direction != 0:
		if !melee_weapon1.visible and !ranged_weapon1.visible and current_state != State.Punch_Walk:
			current_state = State.Walk
			print("125, Walk")
		if melee_weapon1.visible and current_state != State.Sword_Attack_Walk:
			current_state = State.Sword_Walk
			print("128, Sword_Walk")
		if ranged_weapon1.visible and current_state != State.Gun_Walk_Shoot:
		...
		if direction > 0:
			...
			melee_weapon_sprite1.flip_h = false
			tween.tween_property(melee_weapon1, "position", Vector2(50, -10), 0)
		else:
			sprites.flip_h = true
			melee_weapon_sprite1.flip_h = true
			tween.tween_property(melee_weapon1, "position", Vector2(-50, -10), 0)
...
func player_crouch(delta: float):
	if Input.is_action_just_pressed("Crouch"):
		if !is_on_floor():
			...
			current_state = State.Crouch
	elif Input.is_action_just_released("Crouch"): # Switches state from crouch to idle upon release of button
...
func player_jump(delta):
	...
	if is_on_floor() and Input.is_action_just_pressed("Jump"):
		...
		if !melee_weapon1.visible and !ranged_weapon1.visible:
			current_state = State.Jump
			print("168, Jump")
		if melee_weapon1.visible:
	...
	if velocity.x != 0 and !is_on_floor(): # On air + moving
		if !melee_weapon1.visible and !ranged_weapon1.visible:
			if current_state != State.Punch_Jump: # Jump doesn't override punch_jump
				current_state = State.Jump
				print("183, Jump")
			if velocity.x != 0 and Input.is_action_just_pressed("Damage"): # Attack in air
		...
		if ranged_weapon1.visible:
			...
			if velocity.x != 0 and Input.is_action_just_pressed("Damage"): # Attack in air
				...
				timer.start(0.3) # Cooldown for switching states
				print("203, Gun_Shoot_Jump -> Gun_Jump while moving")
...
func melee_attack(delta : float):
	if Input.is_action_just_pressed("Damage") and melee_weapon1.visible:
		if !is_on_floor() and velocity.x == 0:
			current_state = State.Sword_Attack_Jump
			timer.start(0.1) # Cooldown for switching states
			print("269, Sword_Attack_Jump -> Sword_Jump")
		elif is_on_floor():
			if velocity.x == 0:
				current_state = State.Sword_Attack
				timer.start(0.1) # Cooldown for switching states
				print("274, Sword_Attack -> Idle_Sword")
				sfx_saber.play()  # Play the laser sound
			else:
				current_state = State.Sword_Attack_Walk
				timer.start(0.1) # Cooldown for switching states
				print("279, Sword_Attack_Walk -> Sword_Walk")
	
	if Input.is_action_just_pressed("Damage") and !melee_weapon1.visible and !ranged_weapon1.visible:
		if !is_on_floor() and velocity.x == 0:
			current_state = State.Punch_Jump
			timer.start(0.15) # Cooldown for switching states
...
func range_attack(delta : float):
	...
	if Input.is_action_just_pressed("Damage") and ranged_weapon1.visible:
		var bullet_temp = bullet.instantiate() as Node2D
...
func gettingMeleeDamaged():
	if Input.is_action_just_pressed("Damage2") and gettingMeleeHit and !melee_weapon2.visible:
		print("fist")
...
func _on_health_pack_body_entered(body: Node2D) -> void:
	if body.name == "Player1" and isHealthPackOn:
...
func _on_melee_weapon_2_body_entered(body: Node2D) -> void:
	gettingMeleeHit = true
	#print(gettingMeleeHit)

func _on_melee_weapon_2_body_exited(body: Node2D) -> void:
	gettingMeleeHit = false
	#print(gettingMeleeHit)

func deadCheck(): # Shows win screen
	if currentHealth <= 0 and !Player1WinLogo.visible: #Makes sure whoever dies first loses (doesn't display both win screen)
		Player2WinLogo.show()
		queue_free()

func _on_player_2_update_other_player(newIsHealthPackOn) -> void:
...
# "Picks up" Gun
func _on_blue_rifle_body_entered(body: Node2D) -> void:
	ranged_weapon1.show()
	BlueRifle.queue_free()
	BlueSaber.queue_free()

# "Picks up" Sword
func _on_blue_saber_body_entered(body: Node2D) -> void:
	melee_weapon1.show()
	BlueSaber.queue_free()
	BlueRifle.queue_free()
